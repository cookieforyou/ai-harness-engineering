...
将原始业务需求转换为结构化、可验证的需求规格说明书，确保需求的完整性、一致性和可追溯性，为系统设计和开发提供清晰的基线。
...
[THINK] Step 1: 理解业务背景和目标
   ...
   ├─ 验证：与业务方确认理解准确性，目标是否符合SMART原则
   └─ 检查：业务目标是否具体、可衡量、可达成、相关、有时限
   ↓
[ANALYZE] Step 2: 识别干系人和诉求
   ├─ 问：谁是主要干系人？他们各自的诉求和期望是什么？是否有冲突？
   ├─ 验证：列出干系人矩阵，评估影响力和优先级，覆盖所有关键角色类型
   └─ 检查：必须包含决策者、使用者、影响者、监管者四类角色
   ↓
[GATHER] Step 3: 收集和整理需求
   ├─ 问：收集到的需求是否完整？功能需求与非功能需求是否都覆盖？
   ├─ 验证：需求覆盖所有业务场景和用户旅程，使用用户故事格式规范化
   └─ 检查：每条需求符合"As a... I want... so that..."格式
   ↓
[MODEL] Step 4: 构建业务模型
   ├─ 问：业务流程是否清晰？关键决策点和异常处理是否明确？
   ├─ 验证：绘制业务流程图和用例模型，覆盖主要路径和边界情况
   └─ 检查：流程边界清晰，无遗漏的关键路径和异常分支
   ↓
[SPECIFY] Step 5: 编写需求规格
   ├─ 问：每条需求是否符合SMART原则？验收标准是否可测试？
   ├─ 验证：需求可追溯到业务目标，验收标准使用Given-When-Then格式
   └─ 检查：非功能需求完整（性能、安全、可用性、可维护性）
   ↓
[VALIDATE] Step 6: 评审确认并准备交接
   ├─ 组织评审会议，收集干系人反馈
   ├─ 根据反馈修订需求文档，解决发现的冲突和问题
   └─ 获得干系人签字确认，生成交接上下文，准备移交系统设计阶段
```
...
|----|--------|----------|----------|----------|----------|
| DC-001 | 干系人确认完整性 | 干系人识别完成后 | 已覆盖所有关键角色 / 存在遗漏风险 | 必须覆盖决策者、使用者、影响者、监管者四类 | 干系人分析报告 |
| DC-002 | 需求模糊处理 | 发现需求描述不清晰时 | 主动澄清 / 基于假设标注 / 标记为待确认 | 优先主动澄清（最多3轮），无法澄清时明确标注假设 | 需求规格说明书 |
...
| DC-005 | 非功能需求定义 | 功能需求确认后 | 根据系统类型确定NFR范围和指标 | 参考行业标准和类似项目经验值，与架构师协商 | 非功能需求章节 |
...
THEN
  ...
  6. 更新范围基线文档，记录变更原因和影响
  7. 通知所有相关干系人范围变更
END
```

**降级方案**: 将新增需求记录到"未来考虑"清单，不影响当前迭代计划，但需获得产品负责人确认

**升级条件**: 新增需求影响超过20%原定范围，或导致关键里程碑延期，或预算超支超过15%
...
|--------|----------|--------|----------|----------|------|
| KPI-001 | REQ-COVERAGE | ≥95% | (已分析需求数/总需求数) × 100% | 需求清单核对，抽样检查 | 25% |
| KPI-002 | STAKEHOLDER-ID | 100% | (已识别关键干系人/实际关键干系人) × 100% | 干系人矩阵检查，访谈确认 | 20% |
| KPI-003 | AC-CLARITY | ≥90% | (有明确验收标准的需求数/总需求数) × 100% | 验收标准审查，测试映射 | 20% |
| KPI-004 | TRACEABILITY | 100% | (可追溯到业务目标的需求数/总需求数) × 100% | 追溯矩阵检查，逐条验证 | 20% |
| KPI-005 | CONFLICT-RESOLUTION | ≤5% | (未解决冲突数/总冲突数) × 100% | 冲突解决记录审查 | 15% |
