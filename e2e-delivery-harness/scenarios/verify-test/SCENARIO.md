# Testing Scenario

## Purpose

设计和执行测试用例，验证功能正确性，发现并跟踪缺陷，确保软件质量。

## Chain of Thought (思维链)

> AI 执行时的思维引导，帮助逐步完成测试验证工作

### Think-Aloud Protocol

```
THINK: 理解测试范围和策略
   ↓
THINK: 设计测试用例
   ↓
THINK: 准备测试环境
   ↓
THINK: 执行测试用例
   ↓
THINK: 跟踪缺陷修复
   ↓
THINK: 生成测试报告
```

### Step-by-Step Reasoning

**Step 1: 测试策略制定**
- 问：测试策略是否覆盖所有重要场景？
- 验证：功能测试、接口测试、性能测试
- 检查：测试优先级是否合理

**Step 2: 用例设计**
- 问：测试用例是否覆盖所有需求？
- 验证：正向、反向、边界条件
- 检查：用例是否可执行和可重复

**Step 3: 环境准备**
- 问：测试环境是否与生产环境一致？
- 验证：配置、数据、依赖
- 检查：测试数据是否充分

**Step 4: 测试执行**
- 问：所有用例都执行了吗？
- 验证：执行率和通过率
- 检查：缺陷是否正确记录

**Step 5: 缺陷管理**
- 问：缺陷是否都修复并验证了？
- 验证：修复的正确性
- 检查：是否需要回归测试

## Primary Assets

### Agent

- **Agent**: [../../agents/tester.agent.md](../../agents/tester.agent.md)

### Instruction

- **Instruction**: [../../instructions/testing-verification.instructions.md](../../instructions/testing-verification.instructions.md)

### Prompt

- **Prompt**: [../../prompts/verify-test.prompt.md](../../prompts/verify-test.prompt.md)

### Skills

- **Skill**: [../../skills/testing/SKILL.md](../../skills/testing/SKILL.md)

## Error Handling (错误处理)

### EH-1: 测试环境不可用

- **识别信号**：测试环境无法访问
- **处理方式**：
  1. 检查环境状态
  2. 联系运维人员
  3. 记录问题
  4. 标记为 `[环境阻塞]`
- **升级条件**：影响测试进度

### EH-2: 发现严重缺陷

- **识别信号**：发现 P0/P1 缺陷
- **处理方式**：
  1. 立即记录缺陷
  2. 通知开发负责人
  3. 暂停相关测试
  4. 等待修复后重新测试
- **升级条件**：影响发布决策

### EH-3: 测试用例失败

- **识别信号**：测试用例执行失败
- **处理方式**：
  1. 确认测试环境正确
  2. 验证测试数据正确
  3. 确认为代码问题还是测试问题
  4. 记录并创建缺陷
- **升级条件**：缺陷修复率低

## Expected Output

### 产出清单

1. **测试计划**：测试策略和资源规划
2. **测试用例集**：完整的测试用例
3. **测试报告**：测试执行结果和分析
4. **缺陷报告**：发现的缺陷清单

### 输出格式

```markdown
## 测试报告

### 1. 测试概要
...

### 2. 测试结果
...

### 3. 缺陷统计
...

### 4. 测试用例详情
...

### 5. 风险评估
...

### 6. 测试结论与建议
...
```

## Prerequisites

### 必需前置条件

1. 开发任务已完成
2. 测试环境已就绪
3. 需求规格已确认

### 可选前置条件

1. 接口文档
2. 测试规范
3. 历史缺陷数据

## Quality Gates

### 阶段准入

- [ ] 开发产出已提交
- [ ] 测试环境可用
- [ ] 测试数据已准备

### 阶段准出

- [ ] 测试用例全部执行
- [ ] 测试报告已输出
- [ ] 缺陷已跟踪
- [ ] 测试结论明确

## Decision Checkpoints (决策检查点)

| 检查点 | 触发条件 | 等待决策 | 下一步 |
|--------|----------|----------|--------|
| **DC-1: 测试策略确认** | 完成测试计划后 | 测试策略是否合适？范围是否正确？ | 继续用例设计 |
| **DC-2: 测试结论确认** | 完成测试执行后 | 是否可以进入部署阶段？ | 决定下一步 |
| **DC-3: 遗留缺陷确认** | 发现阻塞性缺陷后 | 是否接受遗留缺陷进入部署？ | 修复或继续 |

## Handover Criteria (交接标准)

| 条件项 | 状态 | 说明 |
|--------|------|------|
| 测试计划 | ☐ | 已评审通过 |
| 测试用例 | ☐ | 核心用例覆盖100% |
| 测试报告 | ☐ | 完整且准确 |
| 缺陷清单 | ☐ | 阻塞性缺陷已处理 |

## Workflow

```
1. 启动 → 理解测试范围
2. 测试计划 → 制定测试策略
3. 用例设计 → 设计测试用例
4. 环境准备 → 搭建测试环境
5. 测试执行 → 执行测试用例
6. 缺陷管理 → 跟踪缺陷
7. 测试报告 → 输出测试报告
8. 完成 → 给出测试结论
```

## Related Scenarios

- **Next**: [../deploy-release/SCENARIO.md](../deploy-release/SCENARIO.md) - 部署发布
- **Previous**: [../implement-feature/SCENARIO.md](../implement-feature/SCENARIO.md) - 开发实现

## Metrics

| 指标 | 目标 | 测量方法 |
|------|------|----------|
| 用例执行率 | > 95% | 执行统计 |
| 用例通过率 | > 90% | 通过统计 |
| 缺陷修复率 | > 95% | 缺陷跟踪 |
| 测试覆盖 | > 90% | 覆盖率报告 |
