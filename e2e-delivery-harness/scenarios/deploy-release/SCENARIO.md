...
| 检查点 | 触发条件 | 等待决策 | 下一步 |
|--------|----------|----------|--------|
| **DC-1: 部署策略确认** | 完成部署计划后 | 采用何种部署策略？灰度范围？ | 继续准备 |
| **DC-2: 回滚决策** | 部署过程中出现异常时 | 继续尝试还是立即回滚？ | 回滚或继续 |
| **DC-3: 最终上线确认** | 灰度验证通过后 | 是否接受最终上线？ | 继续监控 |
...
- **Next**: [../monitor-operate/SCENARIO.md](../monitor-operate/SCENARIO.md) - 监控运维
